...
#include <routing/interfacetable.h>
#include <I2CIP.h>
#include "../config.h"

DeviceTable routingtable = DeviceTable();

bool helper_bst_in_ht(DeviceTable& rt, i2cip_device_t* root) {
  if(root->left == nullptr && root->right == nullptr) {
    bool t = rt[root->value]->contains(root);
    char msg[25];
    sprintf(msg, "BST entry %01X.%01X.%01X.%02X in HT", CSOS_FQA_SEG_I2CBUS(root->key), CSOS_FQA_SEG_MUXNUM(root->key), CSOS_FQA_SEG_MUXBUS(root->key), CSOS_FQA_SEG_DEVADR(root->key));
...
bool helper_ht_in_bst(DeviceTable& rt, HashTableEntry<DeviceGroup*>* entry) {
  ...
  for(int i = 0; i < entry->value->numdevices; i++) {
    ...
    char msg[25];
    sprintf(msg, "HT entry %01X.%01X.%01X.%02X in BST", CSOS_FQA_SEG_I2CBUS(fqa), CSOS_FQA_SEG_MUXNUM(fqa), CSOS_FQA_SEG_MUXBUS(fqa), CSOS_FQA_SEG_DEVADR(fqa));
    TEST_ASSERT_TRUE_MESSAGE(t, msg);
    result &= t;
  }
